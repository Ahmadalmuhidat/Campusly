...
export default function PublicProfile() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [profile, societies, posts, events] = await Promise.all([
      ...
      if (events.status === 200) {
        setAttendedEvents(events.data.data || []);
      }
    } catch (error) {
                ...
                {posts.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {posts.slice(0, 5).map(post => (
                      <div key={post.ID} className="rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-300">
                        {post.Image && (
                        ...
                        <div className="p-4">
                          <p className="text-gray-800 leading-relaxed whitespace-pre-line mb-3 text-sm line-clamp-3">{post.Content}</p>
